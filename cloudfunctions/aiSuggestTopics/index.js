...
category 必须从「${CATEGORIES.join(' / ')}」中精确取值；当指定了分类偏好时必须等于所选分类，且候选内容必须真实属于该分类，禁止把其他分类的事件标成所选分类凑数。
{
  ...
  "suggestedDeadline": "停止收注截止时间（必须严格早于结果揭晓时刻：发布会 19:00 就写 18:55，比赛 20:00 开赛就写 19:55，数据 10:00 发布就写 9:55；严禁写成揭晓当天 23:59 或揭晓之后）",
  "resultMoment": "结果揭晓时刻（如：8月18日 19:00 发布会现场宣布；必须晚于 suggestedDeadline）",
  "verifiable": true,
...
11. 【结果时点确定】结果必须绑定固定揭晓时刻（发布会现场、比赛终场、官方数据发布时刻、开奖时刻）；禁止“开分、榜单首更、开奖、销量揭晓”等揭晓时刻不确定的形态。
12. 【截止必须先于揭晓·最关键的规则】答案一旦揭晓，市场就必须已经截止：suggestedDeadline 必须严格早于结果揭晓时刻，严禁写成揭晓当天 23:59 或揭晓之后。示例：发布会 8月18日 19:00 开始并当场公布 → 截止写「8月18日 18:55」，结果揭晓=发布会现场；比赛 20:00 开赛 → 截止写开赛前；官方数据 10:00 发布 → 截止写 9:55。
13. 【禁止提前揭晓型表述】禁止“在 X 日之后 / 一周内 / 未来 N 天是否公布、发布、官宣”这类区间表述（答案可能在区间内提前揭晓）；公布/发布必须绑定固定揭晓时刻（如“在 8月18日 19:00 发布会上是否公布首销日期”）。
14. 【禁止状态类】禁止“是否售罄 / 无货 / 已发布 / 已破 X 亿”等状态可能提前成立的题；禁止“全网 / 各大平台 / 多渠道”等多源表述，状态必须唯一官方来源可判定（如官方商城页）。
15. 【严格分类】每个候选必须真实属于所选分类，category 字段必须与所选分类完全一致；若检索结果没有足够该分类的可验证事件，宁缺毋滥（可少于 10 条），绝不混入汇率/气温/指数/电影票房等其他分类；
16. 【严格依据检索结果】只能基于【联网检索结果】中的事实生成候选，禁止使用你训练记忆中的旧事件（旧日期的票房、指数、气象等）；检索结果未提及的事实不得编造。
17. 【分类级封盘规范】suggestedDeadline 必须按分类遵守最小封盘提前量（截止必须早于数据开始产生/揭晓时刻，防中途数据泄露套利）：
   - 影视娱乐·票房类：截止 = 结算日前一晚（如结算 8月20日 23:59 → 截止 8月19日 23:59:59）；
   - 体育竞技 / 游戏电竞：截止 = 开赛前至少 5 分钟；
   - 财经宏观：截止 = 官方发布前至少 15 分钟；
   - 科技数码（发布会/官宣）：截止 = 发布会开始前至少 15 分钟；
   - 趣味民生（天气/榜单）：截止 = 当日 06:00 或官方发布前；
   - 其余事件：截止必须严格早于结果揭晓时刻至少 5 分钟。`;
  ...
  const list = normalizedParsed
    .filter(c => {
      ...
      // 状态类（售罄/无货等）直接丢弃
      if (['售罄', '无货', '缺货', '断货', '抢购一空'].some(w => title.indexOf(w) >= 0)) return false;
      // 公布/发布类必须绑定固定揭晓时刻（发布会/官宣活动/宣布/揭晓/直播等），否则丢弃
      const announceWords = ['发布', '公布', '官宣', '上市', '开售'];
      const anchoredWords = ['发布会', '官宣活动', '宣布', '揭晓', '仪式', '直播'];
      if (announceWords.some(w => title.indexOf(w) >= 0) && !anchoredWords.some(w => title.indexOf(w) >= 0)) return false;
      // 发布会类截止却写当天 23:59 → 揭晓后才截止，直接丢弃
      const deadline = String(c.suggestedDeadline || '');
      if (title.indexOf('发布会') >= 0 && deadline.indexOf('23:59') >= 0) return false;
