...
9. 【结果时点确定】区分“停止收注截止”与“结果可得时点”：比赛结束、发布会结束、官方统计发布这类结果时点确定的事件可以正常起草（humanReadable 注明判定时点为结果时点，截止可早于结果时点）；若结果时点不确定（如豆瓣开分、榜单首更、开奖），直接返回 {"mode":"manual","provider":"","humanReadable":"该卦题裁决数据发布时间不确定，截止时点可能无数据，禁止发布"}；
10. 【截止前不可揭晓】若卦题是“状态可能提前成立”类（如某产品是否已售罄、是否已发布、是否已破某值），或“无固定揭晓时刻的公布/发布”类（如一周内是否公布、X日之后是否官宣），直接返回 {"mode":"manual","provider":"","humanReadable":"该卦题结果可能在截止前提前揭晓，市场将失去悬念，禁止发布"}；只有具备固定官方揭晓时刻（发布会宣布、官方数据发布、比赛终场）的题才允许起草。
11. 【分类级封盘规范】起草时必须同时给出合理的截止时间（停止收注）与预期结果时刻（官方揭晓），并遵守分类最小封盘提前量：
   ...
   humanReadable 中必须注明截止时点与预期结果时点（如“截止 8月19日 23:59:59，以 8月20日 23:59:59 官方票房快照为准”）。`;
