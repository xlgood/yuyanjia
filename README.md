...
由早期规划《预言大师：本土化轻量级预测市场项目规划书》演进而来，正式产品名定为**「预测卦局」**：用**虚拟爻**支撑的 YES/NO 热点预测，采用**瓜分池（Pari-mutuel）模型**，配套五维荣誉榜单、2 小时异议公示（跨夜顺延）、社区仲裁（仲裁结果即终局）、破产补助等机制，并通过**审核双视图**满足微信提审合规要求。
...
- 事件状态机：`open → locked（截止锁定，等待判定）→ dispute_window（2 小时公示，跨夜顺延）→ resolved`；仲裁结束后直接结算，不再重复公示；
- 自动判定：`resolver` 按 `resolutionSpec` 定时判定（api/weather/webpage 适配器：JSON 接口、天气、官方网页 regex/selector 提取），**backupSources 多源交叉验证**（冲突转人工）、**timestampField 防信息套利**（结果时间早于截止转人工）、锁定时立即触发判定（时效 ~1 分钟）；证据写入 `resolution_logs`，连续失败转人工复核；人工判定走运营后台复核队列；
- **结果版本化**：每次录入/覆写判定 `resultVersion +1`，管理员更正判定后自动给已表态用户补发「结果已更正」订阅消息；
- **通知 Outbox**：判定/仲裁/更正订阅消息先落 `notification_outbox` 再发送，失败由定时器指数退避重试（不再静默丢消息）；
- **SSRF 防护**：数据源 URL 入库与抓取双重校验，禁止内网/回环/链路本地地址；
- 并发与资金安全：发钱口原子防双领、市场/仲裁结算锁、逐注事务结算、过期 PK 定时清理（事务化）、未应战 PK 不再污染榜单；
- 社区仲裁：保证金对赌、24 小时投票、成立翻转/不成立维持、无对赌全额退回、仲裁终局；
- 邀请裂变：邀请码归属（**不透明邀友码 inviteCode**，分享链接不再暴露 openid）、首开加成、首次表态触发奖励（每日上限、事务化）、邀请荣誉可解锁；
- 荣誉体系：20 项成就/榜单荣誉，周/月/PK 按历史快照判定，连胜/总榜实时判定；
- 任务与广告：签到（原子防重复）、看激励视频领爻（每日限次，支持服务端回调 + 内层 sign 校验）、破产补助；
- 运营后台：数据源注册表、发题（AI 起草判定条件；数值型/事实型/网页型三种判定方式）、批量发题、待判定/复核队列（含预期结果时刻）、看板、**定时候选**（勾选一键批量发题、采用/忽略状态流转）；
- **定时选题**（`dailyHotTopics`，每天 08:00/20:00）：聚合 GitHub Trending / Hacker News / IT之家 / 36氪 / 少数派 / 新浪滚动 6 个稳定免费源 → AI 生成候选写入 `topic_candidates` → 管理端确认发题；新闻类素材保留为「素材库」供人工选题；
- AI 选题助手（`aiSuggestTopics`，支持 DeepSeek/千问/Kimi 联网搜索；入口当前按策略隐藏，配置好联网后恢复）；
- 内容安全：微信 `msgSecCheck` + 本地敏感词表兜底（不再 fail-open）；
- 过审合规：全站娱乐声明页脚（首页/详情/我的）、tab「预测」、雅号/修习台用词、**卦友/新人礼爻等占卜化用词弱化**、hero 副标题弱化（收敛占卜联想）；
...
- 工程：git 基线（main 分支）、`scripts/deploy.sh` 一键上传云函数（动态函数列表）、**业务常量单一来源**（`cloudfunctions/_shared/config.js`，`npm run sync:common` 同步）、**CI 质量门禁**（`.github/workflows/ci.yml`：语法 + 冒烟 + 合规 + 常量漂移 + 前后端一致性校验）。
...
- 可选环境变量：`DISPUTE_WINDOW_HOURS`（默认 2）、`LOCK_STALE_HOURS`（默认 24）、`HONORS_CHECK_INTERVAL_MINUTES`（默认 10）、`LEADERBOARD_CACHE_MINUTES`（默认 10）、`ALERT_WEBHOOK_URL`（默认回退 `LOCK_WEBHOOK_URL`）。

**待实现 / 待决策**

- 合规评估（审核双视图、广告+预测组合、仲裁对赌）——产品决策项；
- 提审三件套：隐私保护指引（待起草）、类目选「娱乐-趣味测试/趣味互动」、提审截图用 `compliance` 双视图；
- 二期路线：网页抓取判定（第 2 层）、陪审团仲裁、二级市场交易、Web 运营后台；
- 工程：初始化 git/CI。

## 功能清单

- 首页：6 大分类热点预测流，实时展示双池爻与返还倍数；分页加载更多 + 骨架屏 + 快速切换分类的竞态守卫；锁定中的事件显示「⏸ 等待判定中」；
...
- 我的：爻与连胜资产、我的卦录、破产补助、邀请道友、规则说明；
- 邀请裂变：分享链接带邀请码自动归属，被邀请人首开 +10 爻，其首次表态后邀请人 +5 爻（每日上限防刷，事务化），邀请战绩页实时展示；
- 道友 PK：详情页发起挑战（强制反向立场、爻入池），接受/拒绝，24 小时未应战自动退回（定时清理兜底）；PK 记录中心 + 按胜率排序的 PK 榜；可在「我的」开关是否接受被邀请；
...
- 荣誉墙：20 项成就/榜单荣誉达成即自动解锁（不消耗爻，仅展示与社交炫耀，无兑换现金/实物价值）；
- 用户资料：预设头像（禁止上传）与昵称校验（长度、敏感词、注入拦截，云端叠加微信内容安全检测）；
- 运营后台（仅管理员可见）：数据源注册表维护（先注册后发题，服务端校验）、发布新卦题（AI 起草判定条件）、批量发题、待判定/复核队列、运营看板；
...
locked ──resolver 自动判定 / 运营人工判定──▶ dispute_window（2h 公示，跨夜顺延）
